...
class PdfMergingService
  ...
  def self.add_images_to_pdf(pdf_object, photos_with_context, title:)
    ...
    temp_pdf = Tempfile.new([ "photos", ".pdf" ])
    begin
      Prawn::Document.generate(temp_pdf.path, page_size: "LETTER", margin: 30) do |pdf|
      ...
      # Usar load en lugar de parse para no duplicar la memoria con Strings gigantes en RAM
      pdf_object << CombinePDF.load(temp_pdf.path)
    ensure
      temp_pdf.close!
    end
  ...
  def self.add_signature_annexes(pdf_object, signature_images, inspection)
    ...
    temp_annex = Tempfile.new([ "annex", ".pdf" ])
    begin
      Prawn::Document.generate(temp_annex.path, page_size: "LETTER", margin: 30) do |pdf|
      ...
      # Cargar desde disco reduce picos de RAM
      pdf_object << CombinePDF.load(temp_annex.path)
    ensure
      temp_annex.close!
    end
