class FormFill < ApplicationRecord
  ...
  def generate_unique_photo_attachment_id(field_name)
    return nil if field_name.blank? || self.inspection.blank?
    
    # Formato: inspection_123_field_name_abc123
    random_suffix = SecureRandom.hex(4)
    "inspection_#{self.inspection.id}_#{field_name.parameterize.underscore}_#{random_suffix}"
  end
  ...
  def get_photos_by_field
    ...
    photos.each do |photo|
      ...
      if filename.match(/^inspection_#{inspection.id}_(.+)_[a-f0-9]{8}$/)
        field_parameterized = $1
  ...
  def debug_photos
    ...
    if form_structure.present?
      ...
      photo_fields.each do |field|
        ...
        if matching_photo
          puts "  - Matching filename: #{matching_photo.filename}"
        end
      end
    end
    puts "=== END DEBUG ==="
