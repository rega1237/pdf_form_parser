  ...
  def sync_data
    begin
      ...
      sync_items.each do |item|
        case item[:type]
        when 'form_fill'
          result = sync_form_fill(item)
        when 'inspection'
          result = sync_inspection(item)
        else
          result = { success: false, error: "Tipo de sincronización no soportado: #{item[:type]}" }
        end
  ...
  def sync_status
    ...
    user_inspections = policy_scope(Inspection).includes(:form_fills)
      ...
      status_data = {
        ...
        pending_inspections: user_inspections.where(status: ['pending', 'in_progress']).count,
