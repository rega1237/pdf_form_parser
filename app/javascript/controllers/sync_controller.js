...
export default class extends Controller {
  ...
  disconnect() {
    try {
      if (this.handleOnline)
        document.removeEventListener("app:online", this.handleOnline);
      if (this.handlePendingChanges)
  ...
  async updateSyncStatus() {
    try {
      ...
      const pendingFFIds = new Set(
        (pendingFormFills || []).map((ff) => ff?.id).filter(Boolean),
      );
      const queueFFIds = new Set(
        (queueItems || [])
          .filter(
            (item) =>
              item?.type === "form_fill_update" && item?.form_fill_id != null,
          )
      ...
      const otherQueueItemsCount = (queueItems || []).filter(
      ...
      if (this.hasSyncButtonTarget) {
        if (pendingCount > 0) {
          this.syncButtonTarget.classList.remove(
            "bg-slate-700/50",
            "text-slate-400",
            "cursor-not-allowed",
          );
          this.syncButtonTarget.classList.add(
            "bg-orange-600",
            "hover:bg-orange-700",
            "text-white",
          );
    ...
    try {
      ...
      for (const qi of queueItems || []) {
        if (qi?.type === "form_fill_update" && qi?.form_fill_id != null) {
          ...
          aggregatedChangesByFF.set(qi.form_fill_id, {
            ...prev,
            ...(qi.payload?.changes || {}),
          });
        }
      ...
      for (let i = 0; i < syncItems.length; i++) {
        ...
        this.updateProgressText(
          `Syncing ${item.type} ${i + 1}/${syncItems.length}...`,
        );
        ...
        while (!success && attempts < maxRetries) {
          try {
            ...
            const successItem = (results.success || []).find(
              (s) => s.local_id === item.id,
            );
            const conflictItem = (results.conflicts || []).find(
              (c) => c.local_id === item.id,
            );
            if (successItem) {
              ...
              if (!item.ephemeral) {
                ...
                if (fid) {
                  const allItems = await this.offlineStorage
                    .getAllSyncItems()
                    .catch(() => []);
                  const toRemove = (allItems || []).filter(
                    (si) =>
                      si?.type === "form_fill_update" &&
                      si?.form_fill_id === fid,
                  );
                  for (const r of toRemove) {
                    try {
                      await this.offlineStorage.removeSyncItem(r.id);
                    } catch (e) {}
                  }
                }
              }
              ...
              const fid = item.form_fill_id || item.payload?.id;
              if (fid) {
                await this.offlineStorage.markFormFillAsSynced(
                  fid,
                  item._local_updated_at,
                );
              }
              ...
              if (resolved) {
                ...
                throw new Error(
                  conflictItem?.message || "Conflict not resolved by user",
                );
              }
            } else {
              const errorItem = (results.errors || []).find(
                (e) => e.local_id === item.id,
              );
              const message =
                errorItem?.message ||
                errorItem?.error ||
                "Server did not confirm item sync";
              throw new Error(message);
            }
          } catch (error) {
            console.error(
              `Failed to sync item ${item.id} (attempt ${attempts}):`,
              error,
            );
            ...
            if (error.message.includes("HTTP 4")) {
              console.error(
                "Client error, removing item from sync queue:",
                error.message,
              );
              await this.offlineStorage.removeSyncItem(item.id);
              errorCount++;
      ...
      if (errorCount === 0) {
        this.showNotification(
          `Sync completed: ${successCount} items`,
          "success",
        );
      } else {
        this.showNotification(
  ...
  async handleConflict(conflictItem, originalItem) {
    ...
    if (this.deepEqualNormalized(local, server)) {
      ...
      const successItem = (response?.results?.success || []).find(
        (s) => s.local_id === originalItem.id,
      );
      return !!successItem;
    }
  ...
  async syncItem(item) {
    ...
    const serverType =
      item.type === "form_fill_update" ? "form_fill" : item.type;
