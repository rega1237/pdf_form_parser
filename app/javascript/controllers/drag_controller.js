...
export default class extends Controller {
  ...
  buildItemElement(itemData, globalIndex) {
    ...
    const isChoiceField = itemData.type === "Choice";
  ...
  onSortEnd(event) {
    ...
    // MODIFICADO: Buscar por name o id para compatibilidad
    const movedItem = this.allItems.find(
      (i) => i.name === itemId || i.id === itemId,
    );
    if (!movedItem) return;

    // Find the item in allItems, remove it, then insert it at the new global position
    const itemToMoveIndex = this.allItems.findIndex(
      (i) => i.name === itemId || i.id === itemId,
    );
